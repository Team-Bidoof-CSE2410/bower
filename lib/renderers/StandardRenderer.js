...
StandardRenderer.prototype.error = function (err) {
    ...
    if (this._config.verbose || !err.code || err.errno) {
        ...
        this._write(process.stderr, chalk.magenta('Verbose: ') + str);
        this._write(process.stderr, chalk.magenta('Verbose: ') + new Error().stack);
...
StandardRenderer.prototype._genericLog = function (log) {
    ...
    str = this._prefix(log) + ' ' + log.message + (this._config.verbose ? chalk.gray(' done in ' + log.elapsedTime + 'ms\n') : '\n');
    this._write(stream, (`${log.isVerbose ? chalk.magenta('Verbose ') : ''}${this._config.verbose ? chalk.gray(`${log.time} `) : ''}bower ${str}`));
};
